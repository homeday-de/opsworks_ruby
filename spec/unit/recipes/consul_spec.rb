...
describe 'opsworks_ruby::consul' do
  let(:chef_run) do
  ...
  it "downloads the consul-template binary" do
    expect(chef_run).to create_remote_file_if_missing('/tmp/consul-template.zip')
  end

  it "unzips the consul-template binary" do
    expect(chef_run).to execute('extract_consul_template')
  end
